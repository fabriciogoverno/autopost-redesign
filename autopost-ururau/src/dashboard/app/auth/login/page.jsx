...
export default function LoginPage() {
  ...
  async function onSubmit(e) {
    e.preventDefault();
    setLoading(true);
    // Fake login - aceita qualquer email/senha por enquanto
    setTimeout(() => {
      if (typeof window !== 'undefined') {
        localStorage.setItem('ururau-user', JSON.stringify({ email, name: email.split('@')[0] }));
      }
      router.push('/');
    }, 600);
  }
    ...
    <div className="min-h-screen grid grid-cols-1 lg:grid-cols-2">
      ...
      <div className="flex flex-col justify-between p-8 lg:p-12">
        ...
        <div className="max-w-sm mx-auto w-full">
          ...
          <p className="text-sm text-muted-foreground text-center mb-8">Digite seu email abaixo para acessar sua conta</p>
          ...
          <form onSubmit={onSubmit} className="space-y-4">
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                ...
                <Link href="/auth/forgot" className="text-sm text-primary hover:underline">Esqueceu a senha?</Link>
              </div>
          ...
          <button className="w-full py-2.5 rounded-lg border border-border hover:bg-muted/30 flex items-center justify-center gap-2 text-sm font-medium">
          ...
          <p className="text-center text-sm text-muted-foreground mt-6">
            Não tem uma conta? <Link href="/auth/signup" className="text-primary hover:underline">Criar conta</Link>
          </p>
        </div>

        <div className="flex items-center gap-2 justify-center text-xs">
          {['br','us','es'].map(l => (
            <button key={l} onClick={() => setLang(l)} className={`px-3 py-1 rounded-full ${lang === l ? 'bg-muted' : 'hover:bg-muted/50'} text-muted-foreground`}>
              <span className="text-[10px] font-semibold uppercase mr-1">{l}</span>
              {l === 'br' ? 'Português (BR)' : l === 'us' ? 'English (US)' : 'Español'}
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="hidden lg:flex flex-col items-center justify-center bg-gradient-to-br from-blue-50 to-white p-12">
        ...
        <h3 className="text-2xl font-bold mb-3 text-center">Automatize suas redes sociais</h3>
        <p className="text-sm text-muted-foreground max-w-md text-center">Crie artes profissionais para suas notícias em segundos com templates personalizados.</p>
      </div>
    </div>
  );
}
